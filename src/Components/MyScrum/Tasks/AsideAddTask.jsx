...
function AsideAddTask() {
    ...
    const isAnyFieldEmpty = () => {
        if (taskTitle === '' || taskDescription === '' || taskPriority === '' || taskStartDate === '' || taskLimitDate === '' || taskCategory === '') {
            return true;
        } else {
            return false;
        }
...
const addNewTask = async () => {
    ...
    if (isAnyFieldEmpty()) {
        ...
        const addTask = `http://localhost:8080/backend_proj4_war_exploded/rest/users/${username}/addTask`;
        try {
        ...
        <>
            <aside>
                <div className="add-task-container">
                    ...
                    <input type="text" id="taskName" placeholder="Title (required)" maxLength="15" value={taskTitle} onChange={handleTaskTitle} required/>
                    <textarea id="taskDescription" placeholder="Description (required)" value={taskDescription} onChange={handleTaskDescription} required></textarea>
                    ...
                    <input type="date" id="task-startDate" value={taskStartDate} onChange={handleTaskStartDate} required/>
                    <label className="labels-task-dates" id="label-limitDate">End date</label>
                    <input type="date" id="task-limitDate" value={taskLimitDate} onChange={handleTaskLimitDate} required/>
                    <label className="labels-task-category" id="label-category">Category</label>
                    <select id="task-category" value={taskCategory} onChange={handleTaskCategory}required>
